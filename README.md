...
| **一覧** | 端末内の測位データを**日ごと**に一覧（調査日単位）。読込 / **出力（ZIP 1ファイル）** / 日ごと削除・取込・容量警告。下書きだけは日の中に行として出す |
| **解析** | SkyPlot / SNR / DOP / DRMS。データソースを「ライブ」「読込データ」で切替。読込データはその日の中で地点を選べる |
| **地図** | 地理院地図（標準/淡色/写真）。**load した記録の GNSS 値**を中心マーカー＋エポック点群＋DRMS 円で表示（解析タブと同じ地点選択つき）。ライブ受信中は現在地・精度円・軌跡も表示 |
...
削除は「日ごと（一覧タブ）→ 全消去（設定タブ）」の2段（下書きだけは1件ずつ消せる）。
全消去は失うものを数えて見せ、未書き出しがあるときだけ二度確認する。
...
node ~/.claude/tools/md-to-pdf.mjs docs/funcspec-202609.md   # 変更したものだけ
...
js/
  ...
  list-ui.js         一覧タブ（調査日ごとの一覧・下書きの確定・出力・取込・容量警告）
  loaded-picker.js   読込データの地点セレクタ（解析タブ・地図タブに同じものを置く）
...
| 文書 | 内容 | 主な読者 |
|---|---|---|
| [docs/funcspec-202609.md](docs/funcspec-202609.md) | **機能仕様**（何ができるか・仕様値・画面ごとの定義） | 仕様を確認する人 |
| [docs/usersGuide-202609.md](docs/usersGuide-202609.md) | **利用者の手引**（操作手順・画面の読み方・困ったとき） | 使う人 |
| [docs/design-202609.md](docs/design-202609.md) | **実装設計**（構成・データモデル・モジュール責務・リファクタ記録） | 実装する人 |
| [docs/algospec-202609.md](docs/algospec-202609.md) | **算出・判定ロジック**（精度推定・DRMS/CEP・収束自動停止・表示保持・受信品質・散布図の表示半径） | 数値の根拠を追う人 |
| [docs/hardware-202609.md](docs/hardware-202609.md) | **機器・ファーム仕様**（MAX-M10S / Pico W / UART / BLE / `$PPICO`） | 機器を組む人 |
...
| 解説 | 内容 | 主な読者 |
|---|---|---|
| [docs/record-202609.md](docs/record-202609.md) | **記録タブ機能まとめ**（record → stop → save → load の流れを1枚に） | 記録機能を把握したい人 |
| [docs/list-202609.md](docs/list-202609.md) | **一覧タブ機能まとめ**（日ごとの一覧・出力ZIP の構造・取込） | 出力データを扱う人 |
