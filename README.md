...
3. **save** — 地点名（既定 `yyyy-mm-dd-xx` の同日連番）とメモを付けて**確定**する。
   **このとき初めて地点番号を採番する**（下書きは番号を消費しないので、失敗記録を捨てても番号が飛ばない）。
   確定した記録はそのまま読込データになる。下書きは記録一覧の先頭に出て、再起動後もそこから確定できる。
...
長辺 1280px・1枚 200KB 前後に縮めてから保存する。地点名・メモ・写真は一覧の「編集」で後から直せる。

記録データと写真は端末内に貯まり続けるため、**量を自分で数えて警告する**
（想定上限 100MB、50MB 超で記録タブに警告。記録は止めない）。
warning には未書き出しぶんも併記するので、WiFi のある場所で調査日ごとにバンドルJSON を
書き出し、書き出し済みの調査日を削除して減らす。
