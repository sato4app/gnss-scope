...
- **文書バージョン**：2026.1
- **最終更新**：2026-06-10
- **対象**：現在のコード（`index.html` ＋ `js/`・`css/`）に基づく実装ベースの機能仕様

> PicoW + u-blox MAX-M10S が WebSocket で流す生 NMEA を受信し、測位の有効性・データ品質・
> 衛星の見え方・精度をリアルタイムに可視化し、収録・事後解析するブラウザアプリ。
...
- 構成：単一の `index.html` ＋ ES モジュール（`js/`）。ビルド不要、HTTP 配信で動作。
...
| `js/ws-client.js` | WebSocket接続＋指数バックオフ自動再接続 | 実装済 |
...
設計方針：入力をどこから取ってもパーサ以降を共通化する。ライブ受信もデモも収録の再生も、
...
### 5.1 レイアウト
上部バー（ブランド＋接続状態ドット・WS URL 入力・接続・収録・デモ・接続状態文字・収録状態）
...
- **NFR-6 対応環境**：Chromium 系を主対象（WS/IndexedDB/Canvas）。
